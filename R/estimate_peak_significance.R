...
estimate_peak_significance <- function(num_CG, score, lookup_table){
  ...
  lower_row_sig <- as.numeric(colnames(lookup_table)[max(which(lookup_table[lower_row,-ncol(lookup_table)] <= score ))])
  upper_row_sig <- as.numeric(colnames(lookup_table)[max(which(lookup_table[upper_row,-ncol(lookup_table)] <= score))])
